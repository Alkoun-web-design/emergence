...
export default function ModuleDetails({name, description, image, price, discountPrice, moduleSpeakers, registration, duration} : {name:string, description:ModuleDescription, image:string, price:string, discountPrice:string | null, moduleSpeakers: string[], registration:'Open'|'Closed', duration:string}) {
   ...
   return (
      <>        
            ...
            <div className="shadow-lg grid grid-cols-subgrid col-start-2 col-end-12 bg-gray-50 rounded-lg p-8 text-gray-900">
                ...
                <motion.div 
                  initial={ 'h-100' }
                  animate={ showModuleDetails && 'h-fit' }
                  className={`col-span-full md:col-span-5 ${showModuleDetails ? 'h-fit' : 'h-100 overflow-hidden' } `}>
                   ...
                   {description.overview.map(overview => <p className="my-4 text-gray-600 text-sm md:text-base">{overview}</p>)}
                    <h2 className="text-2xl lg:text-3xl border-b-2 border-transparent border-b-gray-300">
                      Orientation and Focus
                   </h2>
                   {description.orientation.map(orientation => <p className="my-4 text-gray-600 text-sm md:text-base">{orientation}</p>)}
                   <h2 className="text-2xl lg:text-3xl border-b-2 border-transparent border-b-gray-300">
                      Guest Supervisors
                   </h2>
                   {description.guests.map(guests => <p className="my-4 text-gray-600 text-sm md:text-base">{guests}</p>)}
                   <h2 className="text-2xl lg:text-3xl border-b-2 border-transparent border-b-gray-300">
                      Who Is This Series For?
                   </h2>
                   {description.audience.map(audience => <p className="my-4 text-gray-600 text-sm md:text-base">{audience}</p>)}
                </motion.div>
